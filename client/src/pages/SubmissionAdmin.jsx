          ...
          <button
            className={`w-[50%] ${
              view === "pending"
                ...
                : "bg-[#A3A3A3] text-[#444] hover:bg-[#D4D4D4] hover:text-[#1A1A1A]"
            }
           py-4    cursor-pointer`}
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                ...
                <th className="border py-2 bg-sky-500 not-md:hidden">
                  content
                </th>
                <th className="border py-2 bg-sky-500 not-md:hidden">
                  user name
                </th>
                <th className="border py-2 bg-sky-500 not-lg:hidden">
                  submitted at
                </th>
            ...
            <tbody>
              {PendingSubmissions?.data.map((datum) => (
                <tr key={datum.id}>
                  <td className="border py-2 px-2 text-center max-w-[200px] truncate">
                    {datum?.title}
                  </td>
                  <td className="border py-2 px-2 text-center max-w-[200px] truncate not-md:hidden">
                    {datum?.content}
                  </td>
                  <td className="border py-2 px-2 text-center max-w-[200px] truncate not-md:hidden">
                    {datum?.user_name}
                  </td>
                  <td className="border py-2 px-2 text-center max-w-[200px] truncate not-lg:hidden">
                    {(datum?.created_at).slice(0, 10)}
                  </td>
                  <td className="border py-2 px-2 max-w-[200px] truncate">
                    <button
                      className="lg:w-[86px] rounded h-9 px-2 flex justify-center m-auto items-center duration-200 bg-[#06B6D4] hover:bg-[#0891B2] cursor-pointer"
                      onClick={() =>
                        navigate(`/SingleSubmissionPreview/${datum?.id}`)
                      }
                    >
                      details
                    </button>
        ...
        {view === "history" && (
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className="border py-2 bg-sky-500">title</th>
                <th className="border py-2 bg-sky-500 not-sm:hidden">
                  content
                </th>
                <th className="border py-2 bg-sky-500 not-lg:hidden">
                  user name
            ...
            <tbody>
              {AllSubmissions?.data.map((datum) => (
                <tr key={datum.id}>
                  ...
                  <td className="border py-2 px-2 text-center max-w-[200px] truncate not-sm:hidden">
                    {datum?.content}
                  </td>
                  <td className="border py-2 px-2 text-center max-w-[200px] truncate not-lg:hidden">
                    {datum?.user_name}
                  ...
                  <td
                    className={`border py-2 px-2 text-center max-w-[200px] truncate border-[#F5F5F5] not-md:hidden ${
